...
let weatherService = {
    getData: function(city) {
        fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=metric&APPID=a2b4e52032d2ff195e2ec1133387d853`)
            .then(res => res.json())
            .then(res => {
                console.log(res);
...
let uiService = {
    ...
    loadStatistics: function(data) {
        const stats = weatherService.calculateStatistics(data);
        this.statisticsResult.innerHTML = '';
        this.statisticsResult.innerHTML = `
        <div class="row">
            <div class="col-md-6">MAX TEMP: ${stats.temperature.max} C</div>
            <div class="col-md-6">MAX HUMD: ${stats.humidity.max} %</div>
        </div>
        <div class="row">
            <div class="col-md-6">AVG TEMP: ${stats.temperature.avg} C</div>
            <div class="col-md-6">AVG HUMD: ${stats.humidity.avg} %</div>
        </div>
        <div class="row">
            <div class="col-md-6">MIN TEMP: ${stats.temperature.min} C</div>
            <div class="col-md-6">MIN HUMD: ${stats.humidity.min} %</div>
        </div>
    ...
    loadHourlyData: function(data) {
        ...
        for (const item of data.list) {
            this.tableResult.innerHTML += `
            <div class="row">
                <div class="col-md-2"><img src = "http://openweathermap.org/img/w/${item.weather[0].icon}.png" alt="logo"/></div>
                <div class="col-md-2">${item.weather[0].description}</div>
                <div class="col-md-2">${new Date(item.dt * 1000).toDateString()}</div>
                <div class="col-md-2">${item.main.temp.toFixed(0)}C (${item.main.feels_like.toFixed(0)})</div>
...
if(navService.searchInput.value)
